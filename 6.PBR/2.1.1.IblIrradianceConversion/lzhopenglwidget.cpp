...
void LzhOpenGLWidget::paintGL()
{
    ...
    pbr_shader.SetVec3("camPos", cam_pos);

    // render rows*column number of spheres with varying metallic/roughness values scaled by rows and columns respectively
    for (int row = 0; row < nr_rows; ++row)
    {
        pbr_shader.SetFloat("metallic", (float)row / (float)nr_rows);
        for (int col = 0; col < nr_columns; ++col)
        {
            pbr_shader.SetFloat("roughness", clamp((float)col / (float)nr_columns, 0.05f, 1.0f));
            QMatrix4x4 model;
            model.translate(
            ...
            pbr_shader.SetMat3("normalMatrix", normal_matrix.transposed());
            RenderSphere();
        }
    }
    ...
    for (unsigned int i = 0; i < sizeof(light_positions) / sizeof(light_positions[0]); ++i)
    {
        ...
        pbr_shader.SetVec3("lightColors[" + std::to_string(i) + "]", light_colors[i]);
