...
void LzhOpenGLWidget::initializeGL()
{
    ...
    shader.Init(":/shader/6.1.cubemaps.vs", ":/shader/6.1.cubemaps.fs");
    skybox_shader.Init(":/shader/6.1.skybox.vs", ":/shader/6.1.skybox.fs");
    ...
    // load textures
    cube_texture  = LoadTexture(":/res/container.jpg");
...
void LzhOpenGLWidget::resizeGL(int w, int h)
{
    ...
    skybox_shader.Use();
    skybox_shader.SetMat4("projection", perspective);
}
...
void LzhOpenGLWidget::paintGL()
{
    ...
    /*********************************************************************************
     * 可通过先绘制天空盒子，再打开深度测试绘制箱子物体达到想要的结果，但为了优化一点点性能，
     * 可通过后绘制天空盒子，减少带宽（片段着色器的调用次数），绘制时欺骗深度缓冲天空盒子的距离为最远
     ********************************************************************************/
    ...
    // 让skybox世界坐标位置跟随相机移动，视觉上skybox不移动只能旋转
    view.setRow(3, QVector4D(0.0, 0.0, 0.0, 0.0));
    view.setColumn(3, QVector4D(0.0, 0.0, 0.0, 0.0));
