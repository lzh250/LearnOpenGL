...
void LzhOpenGLWidget::initializeGL()
{
    ...
    glEnable(GL_CULL_FACE); // 剔除面（默认剔除顺时针面）
    glCullFace(GL_FRONT);   // 剔除逆时针面       // 其他选项 GL_BACK、GL_FRONT、GL_FRONT_AND_BACK
    // glFrontFace(GL_CCW); ←告诉OpenGL我们希望将顺时针的面（而不是逆时针的面）定义为正向面。其他选项 GL_CW、GL_CCW
    ...
    floor_texture = LoadTexture(":/res/metal.png");
    transparent_texture = LoadTexture(":/res/window.png");
