...
void LzhOpenGLWidget::initializeGL()
{
    ...
    model_red.translate(QVector3D(-0.75f,  0.75f, 0.0f));
    model_green.translate(QVector3D( 0.75f,  0.75f, 0.0f));
    model_blue.translate(QVector3D(0.75f, -0.75f, 0.0f));
    model_yellow.translate(QVector3D(-0.75f, -0.75f, 0.0f));

    cam_pos   = QVector3D(0.0f, 0.0f,  3.0f);
    cam_front = QVector3D(0.0f, 0.0f, -1.0f);
    cam_up    = QVector3D(0.0f, 1.0f,  0.0f);

    shader_red.Init(":/shader/8.advanced_glsl.vs", ":/shader/8.red.fs");
    shader_green.Init(":/shader/8.advanced_glsl.vs", ":/shader/8.green.fs");
    shader_blue.Init(":/shader/8.advanced_glsl.vs", ":/shader/8.blue.fs");
    shader_yellow.Init(":/shader/8.advanced_glsl.vs", ":/shader/8.yellow.fs");
    ...
    /*************************************************************************************************/
    // 从4.2版本开始增加了在着色器中layout (binding = <绑定点>) uniform XXX 方式进行绑定，更方便。
    // 当然也可通过下面代码进行，会覆盖着色器中的layout设定的绑定点。
    ...
    // glUniformBlockBinding(shader_red   .ID, uniform_block_index_red   , 2);
    // glUniformBlockBinding(shader_green .ID, uniform_block_index_green , 2);
    // glUniformBlockBinding(shader_blue  .ID, uniform_block_index_blue  , 2);
    // glUniformBlockBinding(shader_yellow.ID, uniform_block_index_yellow, 2);
    /*************************************************************************************************/
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo_matrices, 0, 2 * 64);

    shader_red.Use();
    shader_red.SetMat4("model", model_red);
    shader_green.Use();
    shader_green.SetMat4("model", model_green);
    shader_blue.Use();
    shader_blue.SetMat4("model", model_blue);
    shader_yellow.Use();
    shader_yellow.SetMat4("model", model_yellow);
}
...
void LzhOpenGLWidget::resizeGL(int w, int h)
{
    ...
    // 向缓冲里写数据
    glBindBuffer(GL_UNIFORM_BUFFER, ubo_matrices);
    glBufferSubData(GL_UNIFORM_BUFFER, 0, 64, perspective.constData());
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
}
...
void LzhOpenGLWidget::paintGL()
{
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, ubo_matrices);
    glBufferSubData(GL_UNIFORM_BUFFER, 64, 64, view.constData());
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
...
unsigned int LzhOpenGLWidget::LoadTexture(char const *path)
{
...
unsigned int LzhOpenGLWidget::LoadCubemap(vector<std::string> faces)
{
    unsigned int texture_ID;
    glGenTextures(1, &texture_ID);
    ...
    for (unsigned int i = 0; i < faces.size(); ++i)
    {
        ...
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, image.width(), image.height(), 0, GL_BGRA, GL_UNSIGNED_BYTE, image.constBits());
    }
