...
void LzhOpenGLWidget::initializeGL()
{
    ...
    QString backpack_path = QCoreApplication::applicationDirPath() + "/res/backpack/backpack.obj";
    our_shader.Init(":/shader/1.model_loading.vs", ":/shader/1.model_loading.fs");
    normal_shader.Init(":/shader/9.3.normal_visualization.vs", ":/shader/9.3.normal_visualization.fs", ":/shader/9.3.normal_visualization.gs");
    our_model.LoadModel(backpack_path.toStdString());
...
void LzhOpenGLWidget::resizeGL(int w, int h)
{
    Perspective();
    our_shader.Use();
    our_shader.SetMat4("projection", perspective);
    normal_shader.Use();
    normal_shader.SetMat4("projection", perspective);
}
