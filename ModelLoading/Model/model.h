...
private:
    ...
    Mesh ProcessMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for(unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            {
                vertex.tex_coords = QVector2D(0.0f, 0.0f);
            }
    ...
    unsigned int TextureFromFile(const char *path, const string &directory)
    {
        ...
        if (!image.isNull())
        {
            ...
            // else if (image.format() == xxx)
            //     internalformat = GL_RGB;
            // else if (image.format() == QImage::Format_RGB32)
            // {
            //     internalformat = GL_RGBA;或GL_RGB
            //     format = GL_BGRA;
            // }

            glBindTexture(GL_TEXTURE_2D, textureID);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, image.width(), image.height(), 0, format, GL_UNSIGNED_BYTE, image.mirrored().constBits());
